...
int main (int argc, char *argv[]) {
    ...
    // create three queues in here
    // Queue inverted store all strings with url from invertedIndex.txt
    // Queue term store only query term which does have url name
    // Queue url store matched url
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (ValidFile(invert_txt)) {
            while (fgets(invertIndex,1000,invert_txt)!= NULL) {
                ...
                // use strtok to break every word in every line
                enterQueue(term, tk);
            }
        }
        ...
        // open file pagerankList
        // use strtok to break every word with every line
        // get value to order every word
        // then print value by order 
        int line = 1;
